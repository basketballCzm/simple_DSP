...
void parse_apmac_closer_msg(const char* msg)
{
    ...
    for(int i = 0; i < num_macs; ++i, offset += 19)
    {
        ...
        if(shop_id)
        {
            ...
            update_user_arrive_time(2, shop_id, user_id, mac,last_time, now ,is_vip);
            update_shopid_of_user_location(user_id,shop_id, now);
...
int main ()
{
    ...
    conf->set("client.id", uuid().c_str(), errstr);
     ...
     */
    RdKafka::ErrorCode resp = consumer->start(topic, ad_partition, start_offset);
    ...
    while (run)
    {
        RdKafka::Message *msg = consumer->consume(topic, ad_partition, 1000);
     ...
     * Stop consumer
     */
    consumer->stop(topic, ad_partition);
